...
# === Start MoodChat Bot Worker (auto-restart, never dies) ===
log_step_start "Starting MoodChat Bot Worker"
if [ -f "$PROJECT_DIR/run-bot-permanent.sh" ]; then
    # Kill any existing bot wrapper first
    if [ -f "$PROJECT_DIR/worker-permanent.pid" ]; then
        OLD_BOT_PID=$(cat "$PROJECT_DIR/worker-permanent.pid" 2>/dev/null || echo "")
        if [ -n "$OLD_BOT_PID" ] && kill -0 "$OLD_BOT_PID" 2>/dev/null; then
            echo "Stopping existing bot wrapper (PID $OLD_BOT_PID)..."
            kill -TERM "$OLD_BOT_PID" 2>/dev/null || true
            sleep 2
            kill -9 "$OLD_BOT_PID" 2>/dev/null || true
        fi
        rm -f "$PROJECT_DIR/worker-permanent.pid"
    fi
    # Also kill any stale worker node processes
    pkill -f "node.*worker-continuous.js" 2>/dev/null || true
    sleep 1
    echo "Starting bot worker with auto-restart..."
    setsid bash "$PROJECT_DIR/run-bot-permanent.sh" > /dev/null 2>&1 < /dev/null &
    disown
    echo "Bot worker started in background"
fi
log_step_end "Starting MoodChat Bot Worker"
