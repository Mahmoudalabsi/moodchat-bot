...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    log(`Image downloaded: ${buffer.length} bytes, base64: ${base64.length} chars`);

    // Step 2: Test Pollinations Vision (free, no API key)
    log('Step 2: Testing Pollinations Vision API...');
    try {
    ...
    // Step 3: Test Gemini with key from database
    log('Step 3: Testing Gemini Vision with DB key...');
    try {
