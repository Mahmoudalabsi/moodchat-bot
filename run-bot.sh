...
export JOIN_PASSWORD="MOOD2026"

LOG_FILE="/home/z/my-project/bot-polling.log"

echo "[$(date)] Starting bot-polling restart loop..." >> "$LOG_FILE"

while true; do
    echo "[$(date)] Starting bot..." >> "$LOG_FILE"
    npx tsx src/lib/bot-polling.ts >> "$LOG_FILE" 2>&1
    EXIT_CODE=$?
    echo "[$(date)] Bot exited with code $EXIT_CODE. Restarting in 3 seconds..." >> "$LOG_FILE"
    sleep 3
done
